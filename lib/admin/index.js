...
function createAdmin (ctx) {
  ...
  console.log('mongoose', ctx.mongoose);
  console.log('db', ctx.db);
  // var admin = ctx.db.admin( );
...
if (!module.parent) {
  ...
  console.log('args', user, pass);
  var conn = ctx.db = mongoose.createConnection(env.MONGO_ADMIN_URI);
  conn.on('connected', function ( ) {
    console.log('connected', arguments, conn);
    
    var admin = ctx.db.db.admin( );
    console.log('admin', admin);
    var roles = [ { role: 'userAdmin', db: user } ];
    var api = createAdmin(ctx);
    admin.addUser(user, pass, {roles: roles }, function (err) {
        if (!err) console.log('added', user);
        else console.log('ERROR', err, res);
        conn.close( )
      // });
    });
  });
