...
function api (ctx) {
  ...
  function find_by_name (req, res, next) {
    if (!req.account) { next("no account id?"); };
  ...
  function query_multienv (req, res, next) {
    if (!req.site || !req.site.sitename) return next({ok:0, msg: "missing sitename", url: req.url});
  ...
  function suggest (req, res, next) {
    ...
    var mongo = 'mongodb://' + mongo_auth + '@' + mongo_suite.ADDR + '/' + req.account.db.name + '?authSource=admin' + mongo_suite.MONGO_URI_ARGS;
  ...
  function create (req, res, next) {
    ...
    if (req.suggested && req.suggested.mongo && req.suggested.internal_name) {
      ...
      console.log("Something missing", req.suggested);
      var err = "input for creation missing";
      if (err) { return next(err); }
    }
  }
